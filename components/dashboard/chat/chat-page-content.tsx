      ...
      <div className={cn(
        ...
        <div className="p-4 border-b bg-muted/20 space-y-4">
           <div className="flex items-center justify-between">
            <h2 className="text-sm font-bold tracking-tight flex items-center gap-2">
              <MessageCircle className="h-4 w-4 text-primary" /> {t_chat.sidebar.title}
            </h2>
          ...
          <div className="flex items-center gap-1 p-1 bg-muted/40 rounded-xl overflow-x-auto no-scrollbar">
            {Object.keys(t_chat.sidebar.tabs).map((t) => (
              <button key={t} onClick={() => setFilterTab(t)} className={cn(
                ...
                {t === 'unread' && Object.keys(unreadCounts).length > 0 && ` ${Object.keys(unreadCounts).length}`}
              </button>
            ))}
          </div>
          <form onSubmit={handleCreateConv} className="relative group">
            <Input value={newTitle} onChange={(e) => setNewTitle(e.target.value)} placeholder={t_chat.sidebar.newTitle} className="rounded-xl pl-9 pr-9 h-10 bg-background/50 text-xs" />
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          {filteredConversations.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full p-8 text-center opacity-30 italic text-xs">{t_chat.sidebar.noThreads}</div>
          ) : (
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-48 rounded-xl shadow-xl glassmorphism">
                      <DropdownMenuItem onClick={() => handleToggleArchive(conv.id, !!conv.userState?.isArchived)} className="gap-2 text-xs">
                        {conv.userState?.isArchived ? <ArchiveRestore className="h-3.5 w-3.5" /> : <Archive className="h-3.5 w-3.5" />} {conv.userState?.isArchived ? t_chat.actions.unarchive : t_chat.actions.archive}
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => handleTogglePin(conv.id, !!conv.userState?.isPinned)} className="gap-2 text-xs">
                        {conv.userState?.isPinned ? <PinOff className="h-3.5 w-3.5" /> : <Pin className="h-3.5 w-3.5" />} {conv.userState?.isPinned ? t_chat.actions.unpin : t_chat.actions.pin}
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => handleToggleRead(conv.id, !!conv.userState?.isRead)} className="gap-2 text-xs">
                        <CheckCircle2 className="h-3.5 w-3.5" /> {conv.userState?.isRead ? t_chat.actions.unread : t_chat.actions.read}
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => handleToggleFavorite(conv.id, !!conv.userState?.isFavorite)} className="gap-2 text-xs">
                        <Star className={cn("h-3.5 w-3.5", conv.userState?.isFavorite && "fill-primary")} /> {conv.userState?.isFavorite ? t_chat.actions.unfavorite : t_chat.actions.favorite}
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => handleToggleMute(conv.id, !!conv.userState?.isMuted)} className="gap-2 text-xs">
                        {conv.userState?.isMuted ? <Bell className="h-3.5 w-3.5" /> : <VolumeX className="h-3.5 w-3.5" />} {conv.userState?.isMuted ? t_chat.actions.unmute : t_chat.actions.mute}
                      </DropdownMenuItem>
                      <DropdownMenuSeparator />
                      <DropdownMenuItem onClick={() => handleClear(conv.id)} className="gap-2 text-xs text-warning"><Eraser className="h-3.5 w-3.5" /> {t_chat.actions.clear}</DropdownMenuItem>
                      <DropdownMenuItem onClick={() => handleDelete(conv.id)} className="gap-2 text-xs text-destructive"><Trash2 className="h-3.5 w-3.5" /> {t_chat.actions.delete}</DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
                </div>
              </div>
      ...
      <div className={cn("flex-1 flex flex-col bg-card border rounded-3xl shadow-xl overflow-hidden glassmorphism border-primary/10 transition-all", !activeConvId ? "hidden md:flex" : "flex")}>
        {activeConvId ? (
          <>
            <div className="flex items-center justify-between p-4 border-b bg-muted/10 backdrop-blur-md">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[9px] text-green-500 font-bold uppercase tracking-widest mt-0.5">{t_chat.status.active}</p>
                </div>
              </div>
              {isEditingAlias && (
                <div className="absolute right-4 top-16 z-50 bg-background border rounded-xl shadow-2xl p-4 w-56 animate-in slide-in-from-top-2">
                   <p className="text-[9px] font-bold uppercase opacity-50 mb-3">{t_chat.dialogs.nicknames}</p>
                   <div className="space-y-3">
                      <Input value={userNickname} onChange={e => setUserNickname(e.target.value)} placeholder={t_chat.dialogs.userAlias} className="h-8 text-xs" />
                      <Input value={adminNickname} onChange={e => setAdminNickname(e.target.value)} placeholder={t_chat.dialogs.adminAlias} className="h-8 text-xs" />
                      <div className="flex gap-2"><Button size="sm" className="h-7 text-[10px] flex-1" onClick={handleUpdateAlias}>{t_chat.actions.save}</Button><Button size="sm" variant="ghost" className="h-7 text-[10px]" onClick={() => setIsEditingAlias(false)}>{t_chat.actions.cancel}</Button></div>
                   </div>
                </div>
              )}
            </div>
            <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 bg-muted/5 custom-scrollbar">
              {loadingMessages ? (<div className="flex flex-col items-center justify-center h-full opacity-30 gap-2"><Loader2 className="h-5 w-5 animate-spin" /><p className="text-[10px]">{t_chat.status.loading}</p></div>) : 
               messages.length === 0 ? (<div className="flex flex-col items-center justify-center h-full text-center opacity-10"><MessageCircle className="h-12 w-12 mb-2" /><p className="text-xs font-bold uppercase tracking-widest">{t_chat.status.starting}</p></div>) : 
            ...
            <div className="p-4 md:p-6 border-t bg-muted/10">
              <form onSubmit={handleSend} className="relative flex items-center gap-3">
                <Input value={content} onChange={(e) => setContent(e.target.value)} placeholder={t_chat.placeholders.typeMessage} className="flex-1 rounded-xl h-12 pr-12 bg-background/50 border-primary/20 text-sm" disabled={sending} />
          ...
          <div className="flex-1 flex flex-col items-center justify-center gap-4 p-12 text-center opacity-30">
            ...
            <div className="space-y-1"><h3 className="text-lg font-bold">{t_chat.placeholders.selectThread}</h3><p className="text-xs">{t_chat.placeholders.sidebarPrompt}</p></div>
          </div>
        )}
      </div>
    </div>
