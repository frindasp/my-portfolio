...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      const [u, totalMessagesRes, recentRes] = await Promise.all([
        getCurrentUser(),
        ...
        <div className="relative z-10 space-y-4 sm:space-y-6">
          ...
          <div className="pt-2 sm:pt-4 flex flex-col sm:flex-row gap-3 sm:gap-4">
            ...
            <Link href="/dashboard/profile">
              <Button
                variant="outline"
                className="w-full sm:w-auto h-11 sm:h-14 px-6 sm:px-8 rounded-2xl font-bold border-muted-foreground/20 hover:bg-muted transition-all"
              >
                View Profile
              </Button>
            </Link>
          ...
          <div className="p-6 sm:p-8 space-y-6">
            ...
            {recentActivities.map((activity) => (
              <div key={activity.id} className="flex gap-4 items-start group">
                <div className="h-10 w-10 sm:h-12 sm:w-12 rounded-full bg-muted flex items-center justify-center font-bold text-[10px] sm:text-xs shrink-0 border border-muted-foreground/10">
                  {activity.action.slice(0, 2)}
                </div>
                <div className="flex-1 min-w-0 border-b border-primary/5 pb-4 group-last:border-0 last:pb-0">
                  <p className="text-sm font-bold leading-tight group-hover:text-primary transition-colors pr-4">
                    {activity.action.replaceAll("_", " ")}
                  </p>
                  <p className="text-xs text-muted-foreground mt-1">{activity.description || "Tanpa deskripsi"}</p>
                  <p className="text-[11px] text-muted-foreground mt-1.5 flex items-center gap-1.5 font-medium">
                    <Clock className="h-3 w-3" /> {new Date(activity.createdAt).toLocaleString("id-ID")}
                  </p>
                </div>
